...
$(function(){
   $('#start').click( function() {
	...
	$("#userid").val(oiiNexus.USER_ID);
		...
		$('#intro2').html("<p>Please try to answer as best you can, but do not worry if you cannot answer every question. For each correct answer, you will be paid a <strong>bonus of £0.05</strong>.</p><p>For each question, you can <strong>use your mouse to pan and zoom the visualization</strong>. Click and drag to move the visualization, and use the scroll wheel on your mouse to zoom.</p><p>For each question you will have a maximum of " + (oiiNexus.config.maxTime/1000) + " seconds in order to ensure you finish the study in a reasonable amount of time. In order to prevent undue rushing, each question requires you to take at least " + (oiiNexus.config.minTime/1000) + " seconds.</p></div><h1>Network diagrams</h1><div class=\"col1\"><p>A network diagram is a pictorial representation of the connections between entities. One specific example is a network diagram of migration between countries. Each country is represented as a <strong>node (circle)</strong> and an <strong>edge (line)</strong> between two nodes represents a connection. In this task, the nodes will be countries and edges will indicate that a large number of people have moved between the two countries in the past.</p><p>In the example image, there are five nodes (A, B, C, D, and E). In this example, each node represents a country. Each edge (line) signifies that a large number of people have moved between the two countries connected to the edge. By looking at the diagram, we can see that people have moved between, for example, A & B, A & C, and A & E. However, not many people have moved between countries A & D (there is no edge).</p></div><div class=\"col2\"><img src=\"images/network_intro.svg\"></div>");
    		$("#intro2").show();
    		});
});
